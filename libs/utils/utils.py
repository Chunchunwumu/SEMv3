...
def visualize_segmentation_label_process(image, table):
    def draw_segmentation(image, segmentation, color):
        contour = np.array(segmentation, dtype=np.int32)
        image = cv2.polylines(image, [contour], False, color,thickness=3)
        return image
    row_lines_map = table["row_line_masks"].sum(axis=0)
    col_lines_map = table["col_line_masks"].sum(axis=0)
    lines_map = np.clip((row_lines_map + col_lines_map), a_min=0, a_max=1)
    lines_map = np.stack( [lines_map*100, lines_map*255, lines_map*50] ,axis=-1)
    cv2.imwrite("./line.png", lines_map)
    # h,w = lines_map.shape[0:2]
    for cell in table["col_start_bboxes"]:
            image = draw_segmentation(image, cell.reshape(-1,2), (0, 0, 255))
    for cell in table["row_start_bboxes"]:
            image = draw_segmentation(image, cell.reshape(-1,2), (0, 255, 0))
    
    # for cell in table["col_start_center_bboxes"]:
    #         image = draw_segmentation(image, cell.reshape(-1,2), (0, 0, 255))
    # for cell in table["row_start_center_bboxes"]:
            image = draw_segmentation(image, cell.reshape(-1,2), (0, 255, 0))
    image = image/2+lines_map/3
...
def visualize_layout(image, table):
    def draw_segmentation(image, segmentation, color):
        for contour in segmentation:
            ...
            image = cv2.polylines(image, [contour], True, color,2)
        return image
    for cell in table['cells']:
        if 'segmentation' in cell:
            image = draw_segmentation(image, cell['segmentation'], (255, 102, 0))
    return image

def visualize_segmentation_label(image, table):
    def draw_segmentation(image, segmentation, color):
        contour = np.array(segmentation, dtype=np.int32)
        image = cv2.polylines(image, [contour], False, color)
        return image
    for cell in table["row_line_segmentations"]:
            image = draw_segmentation(image, cell, (255, 0, 0))
    for cell in table["col_line_segmentations"]:
            image = draw_segmentation(image, cell, (255, 0, 0))
    for cell in table["col_start_center_bboxes"]:
            image = draw_segmentation(image, cell.reshape(-1,2), (0, 0, 255))
    for cell in table["row_start_center_bboxes"]:
...
def visualize_grids(image, grids):
    def draw_segmentation(image, segmentation, color):
        image = cv2.polylines(image, [segmentation], True, color, 2)
        return image
    color = (0,255,0)
    # color = np.random.randint(0,255+1,(3)).tolist()
    for cell in grids:
        image = draw_segmentation(image, cell.reshape(-1,2).astype(np.int32), color)
    return image


def visualize_grids_merge_token(image, grids, tokens):
    def draw_segmentation(image, segmentation, color):
        image = cv2.polylines(image, [segmentation], True, color, 1)
        return image
    def draw_token(image, segmentation, color):
        # image = cv2.polylines(image, [segmentation], True, color, -1)
        image = cv2.fillPoly(image, [segmentation], color)
        return image
    color = (0,255,0)
    token_colors = [ (255,0,0) ,(0,0,255),(211, 0, 148),(255,0, 245 )  ] # clux
    token_map = np.zeros_like(image).astype(np.uint8)
    # color = np.random.randint(0,255+1,(3)).tolist()
    for cell,token_id in zip(grids, tokens):
        # print(cell)
        image = draw_segmentation(image, cell.reshape(-1,2).astype(np.int32), color)
        token_map = draw_token( token_map, cell.reshape(-1,2).astype(np.int32), token_colors[token_id]   )
    result_image = image//2+token_map//2
    return result_image
...
def get_time_str():
    ...
    time_now_list = [time_now.tm_year,time_now.tm_mon,time_now.tm_mday,time_now.tm_hour,time_now.tm_min]
    time_now_list = map(str,time_now_list)
    time_now_str = '_'.join(time_now_list)
    return time_now_str